...
const THEME_QUERY_PARAM_NAMES = ["theme", "themePreset", "themePresetId"];
const PREVIEW_QUERY_PARAM_NAMES = ["preview", "previewMode", "embed"];
const HIDE_TOPOLOGY_STATUS_QUERY_PARAM_NAMES = ["hideTopologyStatus", "suppressTopologyStatus"];

function readRequestedThemePresetId() {
  if (typeof window === "undefined") {
    return "";
  }
  const params = new URLSearchParams(window.location.search);
  for (const name of THEME_QUERY_PARAM_NAMES) {
    const presetId = normalizeThemePresetId(params.get(name));
    if (presetId) {
      return presetId;
    }
  }
...
function readRequestedPreviewMode() {
  if (typeof window === "undefined") {
    return false;
  }
  const params = new URLSearchParams(window.location.search);
  for (const name of PREVIEW_QUERY_PARAM_NAMES) {
    if (!params.has(name)) {
      continue;
    }
...
function readRequestedHiddenTopologyStatus() {
  if (typeof window === "undefined") {
    return false;
  }
  const params = new URLSearchParams(window.location.search);
  for (const name of HIDE_TOPOLOGY_STATUS_QUERY_PARAM_NAMES) {
    if (!params.has(name)) {
      continue;
    }
    const value = String(params.get(name) || "").trim().toLowerCase();
...
function normalizeExternalViewPreset(value) {
  const normalized = String(value || "").trim().toLowerCase();
  if (["iso", "isometric", "default"].includes(normalized)) {
    return "iso";
  }
  if (normalized === "top") {
    return "z";
  ...
  if (["side", "right"].includes(normalized)) {
    return "x";
  }
  if (normalized === "left") {
    return "xNeg";
...
function createThemeStateFromPresetId(presetId) {
  ...
  return {
    ...
    settings: normalizeThemeSettings(cloneThemePresetSettings(normalizedPresetId))
  };
}

function readInitialThemeSettingsState(customPresets, requestedPresetId = readRequestedThemePresetId()) {
  return createThemeStateFromPresetId(requestedPresetId) || readThemeSettingsState(customPresets);
}

function resolveRequestedColorSchemeId(presetId) {
  const themeState = createThemeStateFromPresetId(presetId);
  if (!themeState) {
    return DEFAULT_COLOR_SCHEME_ID;
  }
  return inferThemeSettingsSceneTone(themeState.settings) === "dark"
    ? DARK_COLOR_SCHEME_ID
    : LIGHT_COLOR_SCHEME_ID;
}
...
function isUrdfAngularJoint(joint) {
  const jointType = String(joint?.type || "").trim().toLowerCase();
  return jointType === "continuous" || jointType === "revolute";
}

function finiteJointValue(value, fallback) {
  const numericValue = Number(value);
  return Number.isFinite(numericValue) ? numericValue : fallback;
}

function buildUrdfPreviewMotionTrajectory(joints, currentJointValues = {}) {
  ...
  for (const joint of movableJoints) {
    ...
    if (!jointName || joint?.mimic || String(joint?.type || "").trim().toLowerCase() === "fixed") {
      continue;
    }

    const angular = isUrdfAngularJoint(joint);
    const fallbackMin = angular ? -180 : -0.1;
    const fallbackMax = angular ? 180 : 0.1;
    const minValue = finiteJointValue(joint?.minValueDeg, fallbackMin);
    const maxValue = finiteJointValue(joint?.maxValueDeg, fallbackMax);
    if (maxValue <= minValue) {
      continue;
    }

    const defaultValue = clampNumber(finiteJointValue(joint?.defaultValueDeg, 0), minValue, maxValue);
    const startValue = clampNumber(finiteJointValue(currentJointValues?.[jointName], defaultValue), minValue, maxValue);
    const span = maxValue - minValue;
    const amplitude = angular
      ? Math.min(Math.max(span * 0.25, 18), 75)
      : Math.min(Math.max(span * 0.25, 0.01), 0.08);
    const center = clampNumber(defaultValue, minValue + (span * 0.15), maxValue - (span * 0.15));
    const highValue = clampNumber(center + amplitude, minValue, maxValue);
    const lowValue = clampNumber(center - amplitude, minValue, maxValue);
    if (Math.abs(highValue - lowValue) <= (angular ? 0.5 : 0.0005)) {
      continue;
    }

    startValues[jointName] = startValue;
    highValues[jointName] = highValue;
    lowValues[jointName] = lowValue;
  }
  ...
  return {
    points: [
      { timeFromStartSec: 0, positionsByNameDeg: startValues },
      { timeFromStartSec: 1.1, positionsByNameDeg: highValues },
      { timeFromStartSec: 2.2, positionsByNameDeg: lowValues },
      { timeFromStartSec: 3.3, positionsByNameDeg: startValues }
    ]
  };
}
...
export default function CadWorkspace({
  ...
  const requestedThemePresetId = useMemo(readRequestedThemePresetId, []);
  const forcedColorSchemeId = useMemo(
    () => resolveRequestedColorSchemeId(requestedThemePresetId),
    [requestedThemePresetId]
  );
  const [themeState, setThemeState] = useState(() => (
    readInitialThemeSettingsState(readCustomThemePresets(), requestedThemePresetId)
  ));
  ...
  const requestedPreviewMode = useMemo(readRequestedPreviewMode, []);
  const requestedHiddenTopologyStatus = useMemo(readRequestedHiddenTopologyStatus, []);
  const [previewMode, setPreviewMode] = useState(() => requestedPreviewMode);
  const [orbitPreviewActive, setOrbitPreviewActive] = useState(() => requestedPreviewMode);
  ...
  const readDefaultThemeState = useCallback(() => (
    createThemeStateFromPresetId(requestedThemePresetId) || readSystemDefaultThemeState()
  ), [readSystemDefaultThemeState, requestedThemePresetId]);

  const handleResetThemeSettings = useCallback(() => {
    setThemeState(readDefaultThemeState());
  }, [readDefaultThemeState]);
  ...
  const applyEntrySessionState = useCallback((key, fileSessionState = null) => {
    ...
    if (themeSlice && !requestedThemePresetId) {
      setThemeState({
  ...
  }, [readDefaultThemeState, readEntrySessionState, requestedThemePresetId]);
  ...
  useEffect(() => {
    ...
    const applySystemTheme = () => {
      applyColorSchemeToDocument(forcedColorSchemeId, document.documentElement, {
    ...
    if (forcedColorSchemeId !== DEFAULT_COLOR_SCHEME_ID) {
      return undefined;
    }
  ...
  const handlePreviewUrdfMotion = useCallback(() => {
    if (!selectedUrdfFileRef || !movableUrdfJoints.length) {
      return;
    }
    ...
    const previewTrajectory = buildUrdfPreviewMotionTrajectory(movableUrdfJoints, startJointValues);
    if (!previewTrajectory) {
      return;
    }
    playUrdfTrajectory(selectedUrdfFileRef, startJointValues, previewTrajectory, startJointValues);
  ...
  }, [setThemeMenuOpen, setTabToolsOpen]);
  ...
  useEffect(() => {
    const handleMessage = (event) => {
      ...
      if (message.type === "text-to-cad:set-preview-mode") {
        if (message.previewMode === false) {
          handleExitPreviewMode();
        } else if (message.previewMode === true) {
          handleEnterPreviewMode();
        }
        return;
      }

      if (message.type === "text-to-cad:set-orbit-preview") {
        setOrbitPreviewActive(message.active !== false);
        if (message.active !== false && !previewMode) {
          handleEnterPreviewMode();
        }
        return;
      }

      if (message.type === "text-to-cad:set-view") {
        const viewPreset = normalizeExternalViewPreset(message.view);
        if (!viewPreset) {
          return;
        }
        setOrbitPreviewActive(false);
        window.requestAnimationFrame(() => {
          explorerRef.current?.focusViewPreset?.(viewPreset);
        });
      }
    };
  ...
  }, [handleEnterPreviewMode, handleExitPreviewMode, previewMode]);
    ...
    >
      <div className="fixed inset-0 z-0">
        <CadRenderPane
          ...
          orbitPreviewActive={orbitPreviewActive}
          ...
          suppressTopologySelectionUnavailableAlert={requestedHiddenTopologyStatus}
          ...
          <div className="flex h-full min-w-0">
            ...
            {selectedFileSheetKind === "urdf" || selectedFileSheetKind === "srdf" || selectedFileSheetKind === "sdf" ? (
              <UrdfFileSheet
                ...
                onPreviewMotion={handlePreviewUrdfMotion}
