...
const CadExplorer = forwardRef(function CadExplorer({
  ...
  floorModeOverride = "",
  previewMode = false,
  orbitPreviewActive = previewMode,
  showViewPlane = true,
  ...
  useImperativeHandle(ref, () => ({
    ...
    focusViewPreset(faceId) {
      const normalizedFaceId = String(faceId || "").trim().toLowerCase();
      if (["", "iso", "isometric", "default"].includes(normalizedFaceId)) {
  ...
  useEffect(() => {
    ...
    clearKeyboardOrbitState(runtime.keyboardOrbitState);
    runtime.previewOrbitEnabled = !!orbitPreviewActive;
    runtime.controls.autoRotate = !!orbitPreviewActive;
    runtime.controls.autoRotateSpeed = PREVIEW_AUTO_ROTATE_SPEED;
    runtime.controls.enabled = true;
    runtime.controls.enableDamping = true;
    runtime.controls.dampingFactor = DEFAULT_DAMPING_FACTOR;
    runtime.interactionState.active = !!orbitPreviewActive;
    if (orbitPreviewActive) {
      cancelCameraTransition(runtime, { scheduleIdle: false });
    } else {
      runtime.scheduleIdleQuality();
    }
    runtime.requestRender();
  }, [orbitPreviewActive, explorerReadyTick]);
