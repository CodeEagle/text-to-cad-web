...
function assertStepArtifactError(entry, code, { skipAssets = false } = {}) {
...
test("scanCadDirectory discovers CAD files directly and infers STEP assets", () => {
  ...
  writeFile(path.join(repoRoot, "workspace/robots/sample_robot.viewer.sdf"), "<sdf version=\"1.12\"><model name=\"sample_robot_viewer\" /></sdf>\n");
  ...
  assert.ok(entryByFile(catalog, "robots/sample_robot.sdf").assets.sdf.url.startsWith("/workspace/robots/sample_robot.viewer.sdf?v="));
  ...
  assert.equal(entryByFile(catalog, "robots/sample_robot.viewer.sdf"), undefined);
...
test("scanCadDirectory ignores stale SDF viewer sidecars", () => {
  const repoRoot = makeTempRepo();
  const sourcePath = path.join(repoRoot, "workspace/robots/sample_robot.sdf");
  const viewerPath = path.join(repoRoot, "workspace/robots/sample_robot.viewer.sdf");
  writeFile(viewerPath, "<sdf version=\"1.12\"><model name=\"stale\" /></sdf>\n");
  writeFile(sourcePath, "<sdf version=\"1.12\"><model name=\"sample_robot\" /></sdf>\n");
  const oldTime = new Date(Date.now() - 10000);
  const newTime = new Date(Date.now());
  fs.utimesSync(viewerPath, oldTime, oldTime);
  fs.utimesSync(sourcePath, newTime, newTime);

  const catalog = scanCadDirectory({ repoRoot, rootDir: "workspace" });
  const entry = entryByFile(catalog, "robots/sample_robot.sdf");

  assert.ok(entry.assets.sdf.url.startsWith("/workspace/robots/sample_robot.sdf?v="));
  assert.equal(entryByFile(catalog, "robots/sample_robot.viewer.sdf"), undefined);
});
...
test("scanCadDirectory accepts visual-only STEP topology without selector data", () => {
  const repoRoot = makeTempRepo();
  const stepHash = writeStep(path.join(repoRoot, "workspace/sample_part/sample_part.step"));
  writeFile(path.join(repoRoot, "workspace/sample_part/.sample_part.step.glb"), topologyGlb({
    stepHash,
  ...
  assert.deepEqual(entry.stepArtifact, {
    ok: true,
    stepHash,
    glbPath: "workspace/sample_part/.sample_part.step.glb"
  });
  assert.deepEqual(Object.keys(entry.assets).sort(), ["glb", "topology"]);
  assert.ok(entry.assets.glb.url.startsWith("/workspace/sample_part/.sample_part.step.glb?v="));
  assert.equal(entry.assets.topology.hash, entry.assets.glb.hash);
  assert.equal(entry.assets.selectorTopology, undefined);
});
