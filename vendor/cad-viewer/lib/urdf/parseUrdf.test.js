...
test("parseUrdf accepts primitive visual geometries", () => {
  const robot = new FakeElement("robot", { name: "primitive_robot" }, [
    new FakeElement("material", { name: "blue" }, [
      new FakeElement("color", { rgba: "0.1 0.2 0.8 1" })
    ]),
    new FakeElement("link", { name: "base_link" }, [
      new FakeElement("visual", {}, [
        new FakeElement("origin", { xyz: "0 0 0.1", rpy: "0 0 0" }),
        new FakeElement("geometry", {}, [
          new FakeElement("box", { size: "0.3 0.2 0.1" })
        ]),
        new FakeElement("material", { name: "blue" })
      ]),
      new FakeElement("visual", {}, [
        new FakeElement("geometry", {}, [
          new FakeElement("cylinder", { radius: "0.05", length: "0.2" })
        ])
      ]),
      new FakeElement("visual", {}, [
        new FakeElement("geometry", {}, [
          new FakeElement("sphere", { radius: "0.04" })
        ])
      ])
    ])
  ]);
  ...
  assert.deepEqual(
    urdfData.links[0].visuals.map((visual) => visual.primitive.type),
    ["box", "cylinder", "sphere"]
  );
  assert.deepEqual(urdfData.links[0].visuals[0].primitive.size, [0.3, 0.2, 0.1]);
  assert.equal(urdfData.links[0].visuals[1].primitive.radius, 0.05);
  assert.equal(urdfData.links[0].visuals[1].primitive.length, 0.2);
  assert.equal(urdfData.links[0].visuals[2].primitive.radius, 0.04);
  assert.equal(urdfData.links[0].visuals[0].color, "#1a33cc");
});
