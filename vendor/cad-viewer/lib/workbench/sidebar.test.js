...
test("URL writers skip history updates when the target URL is unchanged", () => {
  const originalWindow = globalThis.window;
  const replaceCalls = [];
  globalThis.window = {
    location: {
      href: "http://viewer.test/?file=arm.urdf&theme=dark",
      pathname: "/",
      search: "?file=arm.urdf&theme=dark",
      hash: ""
    },
    history: {
...
test("selectedEntryKeyFromUrl redirects legacy hidden URDF preview params to source URDF", () => {
  const originalWindow = globalThis.window;
  globalThis.window = {
    location: {
      search: "?file=.preview%2Ftwo_dof_arm.viewer.urdf"
    }
  };
