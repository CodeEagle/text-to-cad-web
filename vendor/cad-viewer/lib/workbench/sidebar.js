...
function legacyPreviewUrdfSourcePath(urlPath) {
  const normalized = normalizeCadFileQueryParam(urlPath);
  const match = normalized.match(/^(?:(.*)\/)?\.preview\/([^/]+)\.viewer\.urdf$/i);
  if (!match) {
    return "";
  }
  ...
  return dir ? `${dir}/${name}.urdf` : `${name}.urdf`;
}
...
export function selectedEntryKeyFromUrl(entries, { cadRefs = readCadRefQueryParams(), defaultFile = readDefaultCadParam() } = {}) {
  ...
  if (explicitFilePath) {
    ...
    if (match) {
      return fileKey(match);
    }
    const legacyUrdfPath = legacyPreviewUrdfSourcePath(explicitFilePath);
    const legacyMatch = legacyUrdfPath ? findEntryByUrlPath(entries, legacyUrdfPath) : null;
    return legacyMatch ? fileKey(legacyMatch) : "";
  }
