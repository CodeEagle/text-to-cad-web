...
function viewerSdfPathForSdfSource(sourcePath) {
  const stem = path.basename(sourcePath, path.extname(sourcePath));
  return path.join(path.dirname(sourcePath), `${stem}.viewer.sdf`);
}

function freshViewerSdfPathForSdfSource(sourcePath) {
  const viewerPath = viewerSdfPathForSdfSource(sourcePath);
  try {
    const sourceStats = fs.statSync(sourcePath);
    const viewerStats = fs.statSync(viewerPath);
    return viewerStats.mtimeMs + 1 >= sourceStats.mtimeMs ? viewerPath : null;
  } catch {
    return null;
  }
}

function isViewerSdfPath(filePath) {
  return path.basename(String(filePath || "")).toLowerCase().endsWith(".viewer.sdf");
}
...
function createSingleAssetEntry({ repoRoot, rootPath, sourcePath, extension }) {
  ...
  if (kind === "sdf") {
    const viewerSdfPath = freshViewerSdfPathForSdfSource(sourcePath);
    const viewerSdfAsset = viewerSdfPath ? assetForPath(repoRoot, viewerSdfPath) : null;
    if (viewerSdfAsset) {
      assets.sdf = viewerSdfAsset;
    }
  }
  const entry = {
    file: fileRefForSource(rootPath, sourcePath),
...
function collectCadSourceFiles(rootPath, result = []) {
  ...
  for (const entry of entries) {
    ...
    if (SOURCE_EXTENSIONS.has(extension) && !isInlineStepGlbArtifactPath(entryPath) && !isViewerSdfPath(entryPath)) {
