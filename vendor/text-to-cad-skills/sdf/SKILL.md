...
9. After creating or modifying `.sdf` output, always hand the explicit generated path to `$render` when available; `$render` checks/reuses a live viewer and returns a link. Pass the original `.sdf` path. The SDF CLI/render stack may create a sibling `<stem>.viewer.sdf` as an internal CAD Explorer preview sidecar when simulator-only plugins or non-interactive joint types would make the static preview noisy.
