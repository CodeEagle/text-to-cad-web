...
class SdfCliTests(unittest.TestCase):
    ...
    def test_generates_viewer_sidecar_for_simulator_only_sdf_features(self) -> None:
        with tempfile.TemporaryDirectory(prefix="tmp-sdf-") as tempdir:
            source_path = Path(tempdir) / "sample_robot.py"
            _write_sdf_source(
                source_path,
                "\n".join(
                    [
                        "return '''<sdf version=\"1.12\"><model name=\"sample\">",
                        "<link name=\"base_link\" /><link name=\"caster\" />",
                        "<joint name=\"caster_ball\" type=\"ball\"><parent>base_link</parent><child>caster</child></joint>",
                        "<plugin name=\"drive\" filename=\"gz-sim-diff-drive-system\" />",
                        "</model></sdf>'''",
                    ]
                ),
            )
            ...
            preview_path = source_path.with_name("sample_robot.viewer.sdf")
            preview = preview_path.read_text(encoding="utf-8")
            self.assertIn('type="fixed"', preview)
            self.assertNotIn("<plugin", preview)
