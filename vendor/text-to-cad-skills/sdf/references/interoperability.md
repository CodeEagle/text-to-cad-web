...
Pass explicit generated or modified original `.sdf` paths to `$render` whenever it is available, and return the live viewer link it prints. For visual feedback, prefer `$render` snapshots over opening the viewer manually or using Playwright.

CAD Explorer renders SDF as static structure plus direct inspection controls. It does not execute simulator plugins or consume file-authored motion contracts. When a sibling `<stem>.viewer.sdf` exists and is fresh, the render stack treats it as an internal preview asset for the original `.sdf`; this sidecar may remove simulator plugins and collapse static-preview-only joint types to fixed without changing simulator output.
