...
CAD Explorer is a static review surface for SDF. The bundled validator checks generic structure only; it does not validate simulator plugins, controller behavior, or sensor runtime contracts.

After generated `.sdf` files are created or modified, hand explicit original paths to `$render` for live viewer links when available. The SDF CLI/render stack may create a sibling `<stem>.viewer.sdf` sidecar for CAD Explorer only, stripping simulator plugins and rendering unsupported static-preview joint types as fixed. Use `$render` snapshots for visual feedback rather than manual viewer or Playwright inspection; do not generate GIFs for SDF review.

Do not represent CAD Explorer preview sidecars as Gazebo physics/controller behavior. They are visualization artifacts; the original `.sdf` remains the simulator artifact.
