...
CAD Explorer reviews SDF files as static model/world structure through `$render` links and still snapshots. Do not add Explorer-only motion plugins; use simulator-native controllers, plugins, or test harnesses for simulator behavior. If CAD Explorer needs a cleaner static preview, rely on the render stack's internal `<stem>.viewer.sdf` sidecar rather than changing simulator SDF semantics.
