...
- For SDF, return/open the original `.sdf` path. CAD Explorer treats a fresh sibling `<stem>.viewer.sdf` as an internal preview asset when present and hides that sidecar from the file catalog.
