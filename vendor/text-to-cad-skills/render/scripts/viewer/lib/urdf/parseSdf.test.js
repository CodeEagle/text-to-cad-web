...
test("parseSdf renders simulator-only joint types as fixed for static preview", () => {
  const staticJointTypes = ["ball", "gearbox", "revolute2", "screw", "universal"];
  const sdfData = parseWithRoot(sdfRoot([
    el("model", { name: "robot" }, [
      link("base_link"),
      ...staticJointTypes.flatMap((type, index) => {
        const childLink = `${type}_link`;
        return [
          link(childLink, { parent: "base_link" }),
          el("joint", { name: `${type}_joint`, type }, [
            textEl("parent", "base_link"),
            textEl("child", childLink),
            textEl("pose", `${0.2 + index * 0.05} 0 -0.1 0 0 0`, { relative_to: "base_link" })
          ])
        ];
      })
    ])
  ...
  assert.equal(sdfData.joints.length, staticJointTypes.length);
  for (const type of staticJointTypes) {
    const joint = sdfData.joints.find((candidate) => candidate.name === `${type}_joint`);
    assert.equal(joint?.type, "fixed");
    assert.equal(joint?.sdf.originalType, type);
    assert.equal(joint?.sdf.renderedAsFixed, true);
  }
  assert.deepEqual(sdfData.sdf.staticMetadata.unsupportedJointTypes, staticJointTypes);
  assert.ok(sdfData.sdf.staticMetadata.warnings.some((warning) => warning.includes('"ball", "gearbox"')));
});
