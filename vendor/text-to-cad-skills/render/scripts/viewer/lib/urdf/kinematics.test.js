...
test("URDF mesh data builds primitive visual geometry without external mesh assets", () => {
  const urdfData = {
    rootLink: "base_link",
    rootWorldTransform: translationTransform(0, 0, 0),
    links: [
        ...
        visuals: [
          {
            id: "base_link:box",
            label: "box",
            primitive: { type: "box", size: [0.3, 0.2, 0.1] },
            color: "#2b2f33",
            localTransform: translationTransform(0, 0, 0)
          },
          {
            id: "base_link:cylinder",
            label: "cylinder",
            primitive: { type: "cylinder", radius: 0.05, length: 0.2 },
            localTransform: translationTransform(0.5, 0, 0)
          }
        ]
      }
    ],
  ...
  assert.equal(meshGeometry.parts.length, 2);
  assert.equal(meshGeometry.parts[0].id, "base_link:box");
  assert.equal(meshGeometry.parts[1].id, "base_link:cylinder");
  assert.ok(meshGeometry.vertices.length > 0);
  assert.ok(meshGeometry.indices.length > 0);
  assert.deepEqual(meshGeometry.parts[0].sourceBounds, {
    min: [-0.15, -0.1, -0.05],
    max: [0.15, 0.1, 0.05]
  });
  assert.equal(meshGeometry.has_source_colors, true);
...
test("URDF mesh data builds sphere primitives", () => {
  const urdfData = {
    rootLink: "base_link",
    rootWorldTransform: translationTransform(0, 0, 0),
    links: [
        ...
        visuals: [
          {
            id: "base_link:sphere",
            label: "sphere",
            primitive: { type: "sphere", radius: 0.2 },
            localTransform: translationTransform(0, 0, 0)
          }
        ]
      }
  ...
  assert.ok(meshGeometry.vertices.length > 0);
  assert.ok(meshGeometry.indices.length > 0);
  assert.deepEqual(rounded(meshGeometry.parts[0].sourceBounds.min), [-0.2, -0.2, -0.2]);
  assert.deepEqual(rounded(meshGeometry.parts[0].sourceBounds.max), [0.2, 0.2, 0.2]);
});
